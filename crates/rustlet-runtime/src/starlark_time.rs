...
impl fmt::Display for StarlarkTime {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if self.utc_offset_secs == 0 {
            ...
            write!(
                f,
                "{year:04}-{month:02}-{day:02}T{hour:02}:{min:02}:{sec:02}{sign}{oh:02}:{om:02}"
            )
        }
    }
}
...
fn tz_offset_at(name: &str, unix_secs: i64) -> anyhow::Result<i32> {
    ...
    let tz =
        jiff::tz::TimeZone::get(name).map_err(|_| anyhow::anyhow!("unknown timezone: {name}"))?;
